...
* #101: added configurable command-line log levels, optional file logging, optional Linux systemd journal output when built with `libsystemd`, and an in-app recent-log viewer with copy/clear actions; per-sink in-app logging controls are still not implemented.
