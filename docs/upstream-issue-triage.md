...
* #288 / #300: the OAuth login URL and scopes now use the current Twitch authorization host, the logged-in user response parser handles Helix `Get Users` responses, chat block/unblock plus blocked-user list calls now use Helix, the followed-channel list now uses Helix `Get Followed Channels`, authenticated stream-status checks now use Helix `Get Streams`, authenticated channel search now uses Helix `Search Channels`, and authenticated game/category browsing now uses Helix `Get Top Games`, `Search Categories`, `Get Games`, and game-filtered `Get Streams`. Follow/unfollow mutations, unauthenticated app-token handling, and remaining browsing calls still need a full Helix migration.
...
* #278, #271, #240, #226, #212, #210, #187, #74, #45.
