...
* #216: added FreeBSD SSL-runtime troubleshooting for the reported `SSL_CTX_set1_groups` startup failure, and startup now logs Qt build/runtime SSL library details when Qt reports SSL support is unavailable; the original FreeBSD 11 crash has not been reproduced in this environment.
