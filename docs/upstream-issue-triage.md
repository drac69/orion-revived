...
* #271: returning to the VOD view no longer forces the grid back to the
  beginning, the loaded VOD list can now be filtered by
  title/game/language/description/date/type text, the loaded list can be
  reversed oldest-first, the player can queue the current filtered VOD list
  with automatic end-of-VOD advance, local VOD progress writes are explicitly
  synced and rewrite the complete saved-position array,
  archive/highlight/upload/all type filters use Helix `Get Videos`, loaded VOD
  pages are tagged so stale channel/type/search-generation replies cannot merge
  into a newer VOD view, loaded VOD pages are cached per channel/type while
  refreshing with configurable cache expiry, supported Helix VOD metadata such
  as description, language, published time, URL, and muted sections is
  preserved in the VOD model, tooltip, cache, and player handoff, muted VOD
  sections from Helix metadata are shown/filterable in the grid, marked on the
  VOD timeline, and skippable from the player controls, and VOD playback
  positions are mirrored to an atomic JSON snapshot in the app data folder for
  folder-level backup/sync and startup recovery with the path exposed in
  settings. CI now guards filter/reverse/type/queue controls, VOD metadata
  roles and parsing, cache preservation, stale reply rejection, muted-section
  playback routing, and atomic progress snapshots. Broad public chapter/tag
  filtering and Twitch cloud sync remain because Twitch's documented `Get
  Videos` response does not expose chapter titles or tag metadata for arbitrary
  VODs, and documented stream markers are scoped to the owning
  broadcaster/editor token.
* #119: the README now explains that the GitHub build commands are terminal commands, separates build/run/install steps, and documents that revalidated Windows/macOS installers are not published yet. The macOS bundle metadata now uses the fork's bundle identifier, current version, high-DPI flag, and no obsolete broad App Transport Security exception, GitHub Actions validates a macOS Qt 5/mpv source build with Homebrew `qt@5` and `mpv` on `macos-15`, and GitHub Actions validates a Windows Qt 5/mpv source build with MSYS2 UCRT64 on `windows-2025-vs2026`.
* #210: new Windows installs now default to ANGLE D3D11 instead of the older D3D9 renderer, and saved D3D9 defaults are migrated to D3D11 to reduce exposure to the Fraps/Qt render-thread crash path; CI guards the D3D11 default, the saved-D3D9 migration, and the explicit D3D9/WARP fallback selectors, and the MSYS2 UCRT64 Windows source build now compiles in GitHub Actions. The Fraps-specific crash has not been reproduced in this environment.
* #90: focused editable QML text controls now request the Qt input method, and Windows tablet/slate systems also launch the OS touch keyboard (`TabTip.exe`/`osk.exe`) when text inputs gain focus; CI guards the shared input-method helper, Windows touch-keyboard discovery, and focus activation for the search, VOD filter, chat, emote filter, and options text inputs. The Windows tablet behavior has not been reproduced in this environment.
* #243: added an opt-in live low-latency playlist request flag (`fast_bread=true`) for live HLS requests, and the mpv backend applies mpv's documented `low-latency` runtime profile for live streams while restoring it for VOD/default playback; CI guards the setting, Twitch query flag, live-only mpv profile application, and restore path. The heavier Twitch prefetch-segment proxy remains unimplemented because the upstream prototype was reported as skippy and needing a security review.
* #202: Android Back key presses from the player view now return to the last non-player tab instead of leaving the player stuck in place, and CI guards that QML Back-navigation contract. The Android Activity now uses `FLAG_KEEP_SCREEN_ON` instead of a deprecated wakelock, the native bridge methods use screen-on names instead of WakeLock terminology, Qt background running is disabled so playback is suspended when Android backgrounds the Activity, Android now participates in screen-density detection before chat/emote providers are initialized so high-density devices request 2x Twitch/BTTV/FFZ/Bits/badge assets, and mobile player surface taps reveal controls without toggling pause so touch misses do not pause the stream. A disabled F-Droid metadata scaffold now points at this maintained fork, is pinned to the current release tag, and is guarded so it cannot accidentally publish while the Android/F-Droid release channel is retired. Android playback pause/resume, call-audio behavior, crash reproduction, emote rendering, Play Store packaging, and a reproducible Android build recipe require a future restoration proposal with target-device/toolchain validation.
...
Twitch documents Helix metadata APIs and embeddable players, but not a supported native HLS playback-token API. The maintained fork therefore classifies native HLS as best-effort only, keeps the direct twitch.tv fallback visible for live channels and VODs, and tracks the source inventory in [docs/twitch-api-surface.md](twitch-api-surface.md). Validation evidence for that product decision is recorded in closed fork [issue #1](https://github.com/belagrf/orion-revived/issues/1).
...
These are packaging/distribution requests or platform-specific reports that need maintainers with those target systems. The maintained-fork release-channel decision is [issue #2](https://github.com/belagrf/orion-revived/issues/2): Android/F-Droid publishing is retired and the disabled metadata remains source-level only. Desktop releases are intentionally source-only; that release-channel decision is recorded in [issue #3](https://github.com/belagrf/orion-revived/issues/3).
...
* #267 / #261: Android source metadata now follows the fork version, CI validates the Android manifest/version/lifecycle/permission invariants that can be checked without an Android toolchain, the legacy Android helper no longer downloads prebuilt OpenSSL shared libraries, and Android/F-Droid release status is documented in `docs/android.md`; publishing a Play Store, APK, or F-Droid package is retired for this fork unless a future maintainer supplies a maintained Android build recipe and target-device validation.
