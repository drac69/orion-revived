...
* #271: returning to the VOD view no longer forces the grid back to the beginning, the loaded VOD list can now be filtered by title/game/date text, the loaded list can be reversed oldest-first, and the player can queue the current filtered VOD list with automatic end-of-VOD advance; broader tag/chapter filtering, muted-section UI, cloud sync, and cache work remains.
