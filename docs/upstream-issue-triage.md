...
* #288 / #300: the OAuth login URL and scopes now use the current Twitch authorization host, the logged-in user response parser handles Helix `Get Users` responses, chat block/unblock plus blocked-user list calls now use Helix, the followed-channel list now uses Helix `Get Followed Channels`, stream-status checks now use Helix `Get Streams`, channel search now uses Helix `Search Channels`, game/category browsing now uses Helix `Get Top Games`, `Search Categories`, `Get Games`, and game/language-filtered `Get Streams`, VOD listing now uses Helix `Get Videos`, authenticated chat badge metadata now uses Helix `Get Channel Chat Badges` and `Get Global Chat Badges`, emote-set loading now uses Helix `Get Emote Sets`, and Bits/Cheermote metadata now uses Helix `Get Cheermotes`. The removed Twitch follow/unfollow mutation endpoints are no longer called; logged-in users are sent to twitch.tv for those actions while unauthenticated local favourites still work. Unauthenticated browsing now fails fast instead of calling deprecated Kraken/v5 metadata endpoints because safe app-token client-credentials handling requires a server-side client secret. VOD playback/chat, legacy badge compatibility calls, and any remaining app-token work still need a deeper replacement.
...
* #271: returning to the VOD view no longer forces the grid back to the beginning, the loaded VOD list can now be filtered by title/game/date text, the loaded list can be reversed oldest-first, the player can queue the current filtered VOD list with automatic end-of-VOD advance, local VOD progress writes are explicitly synced, and the first VOD page is cached per channel while refreshing; broader tag/chapter filtering, muted-section UI, cloud sync, and deeper cache work remains.
