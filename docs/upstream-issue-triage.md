...
These are packaging/distribution requests or platform-specific reports that need maintainers with those target systems. The maintained-fork tracker is [issue #2](https://github.com/belagrf/orion-revived/issues/2) for Android/F-Droid target validation. Desktop releases are intentionally source-only; that release-channel decision is recorded in [issue #3](https://github.com/belagrf/orion-revived/issues/3).
