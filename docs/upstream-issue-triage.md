...
* #288 / #300: the OAuth login URL and scopes now use the current Twitch authorization host, the logged-in user response parser handles Helix `Get Users` responses, chat block/unblock plus blocked-user list calls now use Helix, and the followed-channel list now uses Helix `Get Followed Channels`. Follow/unfollow mutations and browsing calls still need a full Helix migration.
