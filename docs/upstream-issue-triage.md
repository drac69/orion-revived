...
* #187: documented multi-backend builds, kept the manual player selector for builds with more than one backend, added runtime fallback that removes a compiled backend from the selector when its QML module fails to load, and wired backend playback errors into the common player error surface.
...
* #239: Fedora's package was built with the Qt Multimedia/GStreamer backend and missing codec/video sink dependencies, while distro comments recommend rebuilding with mpv. The Qt Multimedia backend now forwards MediaPlayer errors into Orion's visible playback error/fallback UI instead of only logging them; producing Fedora packages with the recommended mpv backend remains distro packaging work.
