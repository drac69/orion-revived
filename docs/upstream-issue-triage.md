...
* #271: returning to the VOD view no longer forces the grid back to the beginning, the loaded VOD list can now be filtered by title/game/date text, and the loaded list can be reversed oldest-first; broader tag/chapter filtering, playlist, muted-section, cloud sync, and cache work remains.
