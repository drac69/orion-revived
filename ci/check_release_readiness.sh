...
require_token "$readiness" "Issue #2 is closed" "Android/F-Droid closure status"
require_token "$readiness" "twitch.tv fallback" "Twitch fallback path"
require_token "$readiness" "docs/twitch-api-surface.md" "Twitch API surface inventory link"
require_token "$readiness" "unsupported Twitch playlist-token" "unsupported Twitch native API fallback status"
require_token "$readiness" "Do not ship a Twitch client secret" "Twitch secret packaging warning"
require_token "$readiness" "Issue #1 is closed" "Twitch API closure status"
require_token "$readiness" "https://github.com/belagrf/orion-revived/issues/1" "Twitch validation issue link"
require_token "$readiness" "https://github.com/belagrf/orion-revived/issues/2" "Android release decision issue link"
